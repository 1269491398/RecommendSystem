...
object StreamingRecommender {
  ...
  /**
    * 从广播变量中获取movieId1与movieId2的相似度，不存在、或movieId1=movieId2视为毫无相似，相似度为0 （两个电影间的相似度）
    * @param simHash
    * @param movieId1
    * @param movieId2
  ...
  def createUpdatedRatings(simiHash: scala.collection.Map[Int, scala.collection.Map[Int, Double]], recentRatings: Array[(Int, Double)], candidateMovies: Array[Int]): Array[(Int, Double)] = {
    ...
    for (cmovieId <- candidateMovies; (rmovieId, rate) <- recentRatings) {
      ...
      if (sim > minSimilarity) {
        allSimilars += ((cmovieId, sim * rate))  //sim * rate 相似度乘以对电影的评分
        if (rate >= 3.0) {
  ...
  def main(args: Array[String]) {
    ...
    val ssc = new StreamingContext(sc, Seconds(2))  // Seconds(2)设置批处理时间
    ...
    //kafka的地址
    val brobrokers = "192.168.22.128:9092"
    //kafka的队列名称
