...
object DataLoader {
  ...
  private def storeDataInMongo(movies: DataFrame, ratings: DataFrame, tags: DataFrame)(implicit mongoConf: MongoConfig): Unit = {
    ...
    // 假设MongoDB中有对应的数据库  删除Movie的Collection
  ...
  def main(args: Array[String]): Unit = {
   ...
   val DATAFILE_MOVIES = "F:\\电影推荐\\RecommendSystem\\recommender\\dataloader\\src\\main\\resources\\big\\movies.csv"
    // [userId,movieId,rating,timestamp]
    val DATAFILE_RATINGS = "F:\\电影推荐\\RecommendSystem\\recommender\\dataloader\\src\\main\\resources\\big\\ratings.csv"

    // [userId,movieId,tag,timestamp]
    val DATAFILE_TAGS = "F:\\电影推荐\\RecommendSystem\\recommender\\dataloader\\src\\main\\resources\\big\\tags.csv"
    ...
    /*
        ...
        tags: tag1|tag2|tag3......
     */
    // 以竖线为分隔符聚合tag，重新起名为tags，然后把mid和tags选出来
    //将tagDF中的标签合并在一起(聚合成一个tags)
    val tagCollectDF = tagDF.groupBy($"mid").agg(concat_ws("|", collect_set($"tag")).as("tags"))

    //将tags合并到movie数据集中产生新的movie数据集(左外连接)
